...
@Component({
  ...
  templateUrl: './consolecosting.component.html',
  providers: [ConsoleCostingService]
})
export class ConsoleCostingComponent {
  // Local Variables 
  title = 'Console Costing';
  ...
  sRecord: Costingd = new Costingd;
  constructor(
    private modalService: NgbModal,
    private mainService: ConsoleCostingService,
  ...
  Process() {
    ...
    let SearchData = {
      ...
      company_code: this.gs.globalVariables.comp_code,
      branch_code: this.gs.globalVariables.branch_code,
      agent: this.Record.cost_agent_name == null ? "" : this.Record.cost_agent_name,
      cntrs: this.Record.cost_book_cntr == null ? "" : this.Record.cost_book_cntr
    };
    ...
    this.mainService.Process(SearchData)
      .subscribe(response => {
        ...
        this.Record.cost_buy_pp = response.buypp;
        this.Record.cost_buy_cc = response.buycc;
        this.Record.cost_buy_tot = response.buytot;
        this.Record.cost_sell_pp = response.sellpp;
        this.Record.cost_sell_cc = response.sellcc;
        this.Record.cost_sell_tot = response.selltot;
        ...
        if (response.format == "RITRA") {
          ...
          if (this.Record.cost_profit > 0)
            this.Record.cost_our_profit = this.Record.cost_profit;
          else
            this.Record.cost_your_profit = this.Record.cost_profit
        } else { //Traffic tech
          this.Record.cost_profit = response.tprofit; //total profit
          this.Record.cost_our_profit = response.sprofit;//shared profit
          this.Record.cost_your_profit = response.sprofit;
        }
  ...
  showeditor(costeditor: any, srec: Costingd) {
    this.sRecord = srec;
    this.open(costeditor);
  }
  ModifiedEditorRecords(params: any) {
    ...
    if (params.saction == "SAVE") {
      ...
      this.Record._globalvariables = this.gs.globalVariables;
      this.Record.DetailList2 = this.RecordDetList;
      this.mainService.GenerateCostingDet(this.Record)
        .subscribe(response => {
          this.loading = false;
          this.Record.cost_buy_pp = response.buypp;
          this.Record.cost_buy_cc = response.buycc;
